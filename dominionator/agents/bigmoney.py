...
class SmithyBigMoneyAgent(dma_base.Agent):
    @staticmethod
    def _check_selected(selected: str, allowed: Set[str]):
        if selected not in allowed:
            raise ValueError(
                f"SmithyBigMoney agent wants to play {selected} but is not an option"
            )
        return selected
    ...
    def get_input_play_action_card_from_hand(self,
                                             ...
                                             board: dmb.BoardState,
                                             allowed: Set[str]) -> str:
        # Cards to play, in order of preference (just a smith)
        pref_order = [
            dmcl.SmithyCard.shortname
        ]
        return [c for c in pref_order if c in allowed][0]
    ...
    def get_input_buy_card_from_supply(self,
        ...
        if dmcl.SmithyCard.shortname not in player.all_cards_names():
            # Buy a smithy over a silver if we don't have one
            pref_order[2:2] = [dmcl.SmithyCard.shortname]
